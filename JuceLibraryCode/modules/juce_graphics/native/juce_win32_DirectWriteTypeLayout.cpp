...
namespace DirectWriteTypeLayout
{
                             ...
                             const int textLen, ID2D1DCRenderTarget* const renderTarget, IDWriteFontCollection* const fontCollection)
    {
        ...
        if (font != nullptr)
        {
            // Get DirectWrite Font from Typeface family and Typeface style
            BOOL fontFound = false;
            uint32 fontIndex;
            fontCollection->FindFamilyName (attr.getFont()->getTypefaceName().toWideCharPointer(), &fontIndex, &fontFound);
            ...
            for (uint32 i = 0; i < fontFacesCount; ++i)
            {
                ...
                if (attr.getFont()->getTypefaceStyle() == String (styleName)) break;
            }
